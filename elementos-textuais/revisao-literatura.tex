...
%Conceitos na saúde, na informação, nas humanas de Redes sociais, redes sociais na saúde, a análise de redes sociais na saúde como ferramenta

%A social network is a social structure made up of individuals (or organizations) called "nodes", which are tied
%(connected) by one or more specific types of interdependency, such as friendship, kinship, common interest,
...
% Social network analysis views social relationships in terms of network theory consisting of nodes and ties (also
% called edges, links, or connections). Nodes are the individual actors within the networks, and ties are the
% relationships between the actors. The resulting graph-based structures are often very complex. There can be many
% kinds of ties between the nodes. Research in a number of academic fields has shown that social networks operate on
% many levels, from families up to the level of nations, and play a critical role in determining the way problems are
...
\section{Rede Social}
% Understanding the nature of relationships and connections between entities is
% key towards understanding a variety of phenomena throughout multiple
% disciplines. The concepts of how a disease is spread, or how people are
% influenced by information are all examples of the need to understand and
...
\subsection{Redes Sociais e a Saúde}
...
O sucesso dessas ações vai depender em suma, do vínculo entre profissional e paciente, estabelecendo pactuações e corresponsabilização. Para garantir isso, o profissional deve ter em mente que o momento da consulta é também um momento de educação em saúde.
